<?php
$erro = null;
$header = "<th>ID</th><th>referencia</th><th>cliente</th><th>entrega</th><th>pagamento</th><th>produtos</th><th class='itemPreco'>total</th><th>observação</th><th class='no-printable'></th><th class='no-printable'></th><th class='no-printable'></th><th>display</th>";
$body = "";
...
try {
	...
	$query = "SELECT pedidos.*, clientes.nome, rs_usuarios.nome AS vnome FROM pedidos INNER JOIN clientes ON pedidos.clienteid = clientes.id INNER JOIN rs_usuarios ON rs_usuarios.id = pedidos.vendedorid where pedidos.entregue = 0 and pedidos.pronto = 1 and clientes.negativado = 0 ".$clause_user." ORDER BY entrega ASC";
	$data = $conn->query($query);
		...
		foreach($data as $consulta) {
			...
			$query2 = "SELECT rs_pedidos_produtos.idproduto, rs_pedidos_produtos.cor, produtos.preco, produtos.nome FROM produtos INNER JOIN rs_pedidos_produtos ON rs_pedidos_produtos.idproduto = produtos.id WHERE rs_pedidos_produtos.idpedido = ".$consulta["id"];
			...
			foreach($data2 as $consulta2) {
				...
				$nomesProd.= $consulta2["nome"]." ".$consulta2["cor"].", ";
			...
			$body.= "<tr class='".$styleTr."'>
				...
				<td title='".$consulta["nome"]."'>".$consulta["nome"]."</td>
				...
				<td title='".$consulta["observacao"].$vnome."'>".$consulta["observacao"]."</td>
				<td class='tb-icon' title='Ver'>
					<a class='load-gig' href='".BASE_DIR."?controler=infoItem&item=pedidos&itemid=".$consulta["id"]."&hidebtnprt=true'>
...
$content = str_replace(array("{header}","{body}","{indexMenu}","{url_del}","{id_del_tb}",'data-pagination="true"'),array($header,$body,$rs_menu,$url_del,1,"data-pagination='false'"),$tableFile);
$containerEDT.= $content;
print $containerEDT;
if($contPedidos == 0){
	print "<div class='info-empty-pronto'>Nenhum produto pronto para ser entregue</div>";
}
?>